...
def train_model(data_loader, model, model_path):
    # 设置模型为训练模式
    model.train()
    # todo 1. 准备数据（此处已经传入）
    # todo 2. 准备模型（此处已经传入）
    ...
    # todo 4. 创建优化器
    optimizer = torch.optim.SGD(lr=0.01, momentum=0.9, params=model.parameters())
    # todo 5. 外层循环控制轮次
...
if __name__ == '__main__':
    ...
    # train_model(train_data_loader, model, model_path)
